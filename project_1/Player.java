...
public abstract class Player {
   ...
   // Hand hand;
    //int expose = 0;//allow out side to see?
	
	int exposedToMe = 0 ;
    Hand hand;
    int bankroll;
    int minimum;
    boolean isReshuffled = false;
    boolean isDraw = false;
    boolean needExposed = false;
     int otherCount = 0;
    ...
    public abstract void getCard(Card c,int shuffleTimes);
    
    
    /*Player(int bankroll){//set the initial bankroll?
    	if(bankroll <100) bankroll =100;
    	this.bankroll = bankroll;
    	
    }*/
    
    boolean isBroke() {
    	if (bankroll < minimum) return true;
    	else return false;
    }
    
 /*   public int bet(int bankroll, int minimum){
    	//if(minimum < MINIMUM) minimum = MINIMUM;
    	if(bankroll <= minimum) {
    		System.out.println("Your are broke! hahahaha!  get out!");
    		return -1; 
    	}
    	Scanner sc1=new Scanner(System.in);
    	System.out.println("Your banlance is : "+bankroll+"input your bet ... the smallest is $5 ");
    	int bet = Integer.valueOf(sc1.nextLine());
    	bankroll = bankroll - bet;
    	// means player bankrupt
    ...
    public abstract void cleanInTrick(
    		
    		
    		);
    
    public abstract void cleanToNewRound();
    ...
    public void otherPlayValue(int otherCount) {
